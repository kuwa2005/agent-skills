...
チェック結果の報告も抽象で終わらせない。実値は出さないが、**何を見てどう判定したか**は具体にする。

悪い例（不十分）:
> 秘密はなさそうなのでコミットします。

良い例（このレベルまで求める）:
> Secret check: A–F 実施。`git status` で staged は `src/api.ts`, `README.md` のみ。禁止パスなし。cached diff に `DB_PASS`/`FTP_PASS`/`BEGIN .* PRIVATE KEY`/`AKIA` ヒットなし。`git ls-files` に `.env` / `deploy.sh` なし。結果: **GO**。続けて個別 `git add` 済みファイルのみ commit する。

悪い例（不十分・危険）:
> `.env` に入っているパスワードは `***` ですが形式は普通のやつです。

良い例（このレベルまで求める）:
> `.env` は中身を読んでいない。追跡・ステージ対象外であることを path チェックでのみ確認。値には触れない。

### 図必須（漏洩対応フロー）

履歴書き換え・ローテーションが必要なときは、手順を ASCII / mermaid で示し、**いまどの段か**を明示する。文章の曖昧な「後で直す」は不可。

```
[検知] 追跡パス or diff ヒット（値は書かない）
   │
   v
[NO-GO] git 書き込み停止
   │
   v
[ローテ] ユーザが秘密を無効化（エージェントは実値を扱わない）
   │
   v
[除去] gitignore + rm --cached / filter-repo（置換表はユーザ用意）
   │
   v
[検証] ls-files / gitleaks（ヒットはパスとハッシュのみ）
```
